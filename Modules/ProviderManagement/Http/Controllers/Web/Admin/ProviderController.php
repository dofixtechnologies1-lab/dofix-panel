...
class ProviderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $providers = $this->provider->with(['owner', 'zone'])->where(['is_approved' => 1])->withCount(['subscribed_services', 'bookings'])
            ->when($request->has('search'), function ($query) use ($request) {
                ...
                return $query->where(function ($query) use ($keys) {
                    foreach ($keys as $key) {
                        $query->orWhere('contact_number', 'LIKE', '%' . $key . '%')
            ...
            ->when($category_id, function ($query) use ($category_id) {
                $query->where('service_id', $category_id);
            })
        ...
        return view('providermanagement::admin.provider.index', compact('providers', 'topCards', 'search', 'status', 'categories', 'category_id'));
    }
    ...
    public function create(): Renderable
    {
        ...
        $services = $this->service->ofStatus(1)->get();
        ...
        $categories = $categories->get();
        return view('providermanagement::admin.provider.create', compact('zones', 'commission', 'subscription', 'formattedPackages', 'duration', 'freeTrialStatus', 'services', 'categories'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (User::where('phone', $request['contact_number'])->first()) {
        ...
        if ($request->hasFile('bank_docs')) {
            $bank_docs = file_uploader('provider/bank_docs/', 'png', $request->file('bank_docs'));
        }
        ...
        $provider->service_id = $request->service_name;
        ...
        $this->bank_detail::updateOrCreate(
            ...
            [
                ...
                'acc_holder_name' => $request->account_hold_name,
    ...
    public function edit(string $id)
    {
        ...
        return view('providermanagement::admin.provider.edit', compact('provider', 'zones', 'commission', 'subscription', 'formattedPackages', 'duration', 'freeTrialStatus', 'packageSubscription', 'services', 'categories'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $providerObj = $this->provider->with('owner')->find($id);
        ...
        if (User::where('email', $request['email'])->where('id', '!=', $providerObj->user_id)->exists()) {
            Toastr::error(translate('Email already taken'));
            return back();
        }

        if (User::where('phone', $request['contact_number'])->where('id', '!=', $providerObj->user_id)->exists()) {
            Toastr::error(translate('Phone already taken'));
            return back();
        }
        ...
        // $providerObj = $this->provider;
        $providerObj->company_name = $request->company_name ?? '';
        $providerObj->full_name = $request->full_name;
        $providerObj->contact_number = $request->contact_number;
        $providerObj->alt_contact_number = $request->alt_contact_number ?? '';
        $providerObj->email = $request->email ?? '';
        $providerObj->zone_id = $request->zone_id ?? '';
        $providerObj->coordinates = ['latitude' => $request->latitude, 'longitude' => $request->longitude];

        $providerObj->service_id = $request->service_name;
        $providerObj->adhar_number = $request->adhar_number;

        if (!empty($adhar_img))
            $providerObj->adhar_img = $adhar_img ?? '';

        $providerObj->pan_number = $request->pan_number;

        if (!empty($pan_img))
            $providerObj->pan_img = $pan_img ?? '';

        $providerObj->dl_number = $request->dl_number;

        if (!empty($pan_img))
            $providerObj->dl_img = $dl_img ?? '';

        if (!empty($profile_img))
            $providerObj->profile_img = $profile_img ?? '';

        if (!empty($bank_docs))
            $providerObj->bank_docs = $bank_docs ?? '';

        $owner = $providerObj->owner()->first();


        $owner->user_type = 'provider-admin';

        if ($providerObj->is_approved == '2' || $providerObj->is_approved == '0') {
            $providerObj->is_approved = 1;
            $providerObj->is_active = 1;
            $owner->is_active = 1;
            try {
                Mail::to($providerObj?->owner?->email)->send(new RegistrationApprovedMail($providerObj));
            } catch (\Exception $exception) {
                info($exception);
            }
        }

        DB::transaction(function () use ($providerObj, $owner, $request) {
            $owner->save();
            $owner->zones()->sync($request->zone_id);
            $providerObj->save();
        });
        ...
        if ($request->plan_type == 'subscription_based') {
            $provider_id = optional($providerObj)->id;
            $result = true;
            ...
            if ($packageSubscription === null || $packageSubscription->subscription_package_id != $packageId) {
                ...
                if ($request->plan_price == 'received_money') {
                    ...
                    $payment->success_hook = 'subscription_success';
                    $payment->failure_hook = 'subscription_fail';
                    $payment->payer_id = $providerObj->user_id;
                    $payment->payment_method = 'manually';
                    $payment->additional_data = json_encode($request->all());
                    $payment->attribute = 'provider-reg';
                    $payment->attribute_id = $provider_id;
        ...
        return $this->provider
            ->when($request->has('search'), function ($query) use ($request) {
                ...
                $query->where(function ($q) use ($keys) {
                    foreach ($keys as $key) {
                        $q->orWhere('contact_number', 'LIKE', "%{$key}%")
                            ->orWhere('email', 'LIKE', "%{$key}%")
                            ->orWhere('company_name', 'LIKE', "%{$key}%");
                    }
                });
