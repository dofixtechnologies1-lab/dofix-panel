...
class AccountController extends Controller
{
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        $limitStatus = provider_warning_amount_calculate($provider?->owner?->account?->account_payable ?? 0,$provider?->owner?->account?->account_receivable ?? 0);
        $provider['cash_limit_status'] = $limitStatus == false ? 'available' : $limitStatus;
        if (!empty($provider->service_id)) {
            $category_data = DB::table('categories')
                ->where('id', $provider->service_id)
                ->select('minimum_balance', 'name')
                ->first();
        
            if ($category_data) {
                $category_info = [
                    'minimum_balance' => $category_data->minimum_balance ?? 0,
