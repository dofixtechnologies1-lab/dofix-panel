...
class WithdrawController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $values = json_decode($request['withdrawal_method_fields'], true)[0];
        ...
        if ($receivable > $payable && $payable != 0) {
            ...
            if ($request['amount'] > $totalReceivable) {
                return response()->json(response_formatter(DEFAULT_400, null, 'Please enter an amount less than or equal to the total receivable.'), 400);
        ...
        if ($amount > $account->account_receivable) {
            return response()->json(
                response_formatter(DEFAULT_400, null, 'Insufficient balance. Please enter an amount less than or equal to your total receivable.'),
                400
            );
        }

        if ($amount < $withdrawRequestAmount['minimum']) {
            return response()->json(
                response_formatter(DEFAULT_400, null, 'The amount entered is below the minimum withdrawal limit of ' . $withdrawRequestAmount['minimum'] . '.'),
                400
            );
        }

        if ($amount > $withdrawRequestAmount['maximum']) {
            return response()->json(
                response_formatter(DEFAULT_400, null, 'The amount entered exceeds the maximum withdrawal limit of ' . $withdrawRequestAmount['maximum'] . '.'),
                400
            );
        }
