...
class ProviderController extends Controller
{
     ...
     * @return JsonResponse
     */
    public function dashboard(Request $request, Transaction $transaction, SubscribedService $subscribedService, Serviceman $serviceman)
    {
    ...
    public function getBankDetails(Request $request)
    {
        ...
        $bankDetails->cancelcheck = $this->provider
            ->where('id', $request->user()->provider->id)
            ->select('id', 'bank_docs')
            ->get();
    ...
    public function updateBankDetails(Request $request): JsonResponse
    {
        ...
        if ($request->hasFile('cancelchequeImage')) {
            ...
            $provider->bank_docs = $cancelchequeImage;
            $provider->save();
        }
    ...
    public function updateProfile(Request $request): JsonResponse
    {
        ...
        if ($request->has('logo')) {
            $provider->profile_img = file_uploader('provider/profile_img/', 'png', $request->file('logo'), $provider->logo);
        }
        $provider->company_address = $request->company_address;

        $provider->full_name = $request->contact_person_name;
        $provider->contact_number = $request->contact_person_phone;
        $provider->email = $request->contact_person_email;
        $provider->alt_contact_number = $request->alt_contact_number;


        $provider->adhar_number = $request->adhar_number;
        if ($request->has('adhar_img')) {
            $provider->adhar_img = file_uploader('provider/adhar_img/', 'png', $request->file('adhar_img'), $provider->adhar_img);
        }
        $provider->pan_number = $request->pan_number;
        if ($request->has('pan_img')) {
            $provider->pan_img = file_uploader('provider/pan_img/', 'png', $request->file('pan_img'), $provider->pan_img);
        }
        $provider->dl_number = $request->dl_number;
        if ($request->has('dl_img')) {
            $provider->dl_img = file_uploader('provider/dl_img/', 'png', $request->file('dl_img'), $provider->dl_img);
        }
    ...
    public function Transactions(Request $request)
    {
        ...
        $transactions = $this->transaction->with([
            'booking', 'from_user' => function ($query) {
                $query->select('id', 'first_name', 'last_name');
            },
            'to_user' => function ($query) {
                $query->select('id', 'first_name', 'last_name');
            }
        ])->where('to_user_id', $request->user_id)->get();
    ...
    * @return JsonResponse
    */
    public function pushNotificationProvider(Request $request)
    {
        $validator = Validator::make($request->all(), [
