        ...
        <div class="container-fluid">
            ...
            <div class="row gy-3">
                <div class="col-lg-8">
                        ...
                        <div class="card-body pb-5">
                                    ...
                                    <div class="text-start text-sm-end">
                                        @if($booking->payment_method == 'offline_payment' && $booking->booking_offline_payments->isNotEmpty())
                                            <p class="mb-2"><span>{{ translate('Request Verify Status') }} :</span>
                                                @if($booking->booking_offline_payments?->first()?->payment_status == 'pending')
                                                    <span
                                                        class="text-info text-capitalize fw-bold">{{ translate('Pending') }}</span>
                                                @endif
                                                @if($booking->booking_offline_payments?->first()?->payment_status == 'denied')
                                                    <span
                                                        class="text-danger text-capitalize fw-bold">{{ translate('Denied') }}</span>
                                                @endif
                                                @if($booking->booking_offline_payments?->first()?->payment_status == 'approved')
                                                    <span
                                                        class="text-primary text-capitalize fw-bold">{{ translate('Approved') }}</span>
                                                @endif
                                            </p>
                                        @endif
                                        ...
                                        <p class="mb-2">
                                            ...
                                            <span
                                                class="ms-3 badge badge-{{ $booking->is_paid ? 'success' : 'danger' }}"
                                        ...
                                        <h5 class="d-flex gap-1 flex-wrap align-items-center">
                                            ...
                                            <div id="service_schedule__span">
                                                <div>{{ date('d-M-Y h:ia', strtotime($booking->service_schedule)) }}
                                                    <span
                            ...
                            <div class="table-responsive border-bottom">
                                <table class="table text-nowrap align-middle mb-0">
                                    ...
                                    <tbody>
                                    @php($subTotal = 0)
                                        <?php $con = $agg = $con_p = $agg_p = 0 ?>
                                        <?php
                                        $dueTotal = 0;
                                        ?>
                                    @foreach ($booking->detail as $detail)
                                            <?php
                                            $gstCal = gst_calculation($detail->variation_id);
                                            $con_fee = $gstCal['con_fee'];
                                            $agg_fee = $gstCal['agg_fee'];
                                            $con_per = $gstCal['con_per'];
                                            $agg_per = $gstCal['agg_per'];
                                        ...
                                        <tr>
                                            <td class="text-wrap ps-lg-3">
                                                @if (isset($detail->service))
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        @if($detail->is_addon == 1)
                                                            <span
                                                                class="badge badge-info mb-1">{{ translate('Addon') }}</span>
                                                        @endif
                                            ...
                                            {{-- <td>{{ gst_calculation($detail->variation_id) }}</td>  --}}
                                            <td class="text--end">
                                                <p class="mb-1">Conn
                                                    Fee: {{ with_currency_symbol($con_fee) }}@if($con_per > 0)
                                                        (Inc. {{$con_p }}% GST)
                                                    @endif</p>
                                                <p class="mb-1">Agg
                                                    Fee: {{ with_currency_symbol($agg_fee) }}@if($agg_per > 0)
                                                        (Inc. {{$con_p }}% GST)
                                                    @endif</p>
                                                <p class="mb-1">
                                                    <strong>Total: {{ with_currency_symbol($detail->total_cost) }}</strong>
                                                </p>
                                            </td>
                                        </tr>
                                        @php($subTotal += $detail->service_cost * $detail->quantity)
                                            ...
                                            $con += $gstCal['con_fee'];
                                            $agg += $gstCal['agg_fee'];
                                            $con_p += $gstCal['con_per'];
                                            $agg_p += $gstCal['agg_per'];
                                        ...
                                        @if($detail->is_addon == 1)
                                                <?php
                                                $dueTotal += $detail->service_cost * $detail->quantity;
                                                ?>
                                        @endif
                                    @endforeach
                                    </tbody>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table-md title-color align-right w-100">
                                            ...
                                            {{-- <tr>
                                                ...
                                                ?>
                                            <tr>
                                                <td class="text-capitalize">Conn Fee</td>
                                                <td class="text--end pe--4">
                                                    {{ with_currency_symbol($con) }} </td>
                                            </tr>
                                            <tr>
                                                <td class="text-capitalize">Agg Fee</td>
                                                <td class="text--end pe--4">
                                                    {{ with_currency_symbol($agg) }} </td>
                                            </tr>
                                            <tr>
                                            ...
                                            @endif
                                                ...
                                                <?php
                                                $dueAmount = $dueTotal + $dueAmount;
                                                ?>

                                            @if ($dueAmount > 0  && $booking->due_amt_status == 0)
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <h4 class="mb-2">Message : </h4> @if($booking->message)
                                <p>{{$booking->message}}</p>
                            @else
                                <p>N/A</p>
                            @endif
                    ...
                    <div class="card">
                        <div class="card-body">
                                            ...
                                            @if($booking->booking_offline_payments->isNotEmpty())
                                                <div class="d-flex gap-1 flex-column">
                                                    ...
                                                    @foreach ($booking?->booking_offline_payments?->first()?->customer_information ?? [] as $key => $item)
                                                        <div class="d-flex gap-2">
                                                            @if ($key != 'payment_note' )
                                                                <span
                                                                    class="w-100px d-flex justify-content-start">{{ translate($key) }}</span>
                                                ...
                                                @if($booking->booking_offline_payments?->first()?->payment_status == 'pending')
                                                    <div class="d-flex flex-wrap justify-content-center gap-3 mt-4">
                                                        <button class="btn badge-danger flex-grow-1 py-3"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#deniedModal-{{$booking->id}}">{{ translate('deny') }}</button>
                                                        <button
                                                            class="btn badge-info flex-grow-1 py-3 offline-payment">{{ translate('approve') }}</button>
                                                    </div>
                                                @elseif($booking->booking_offline_payments?->first()?->payment_status == 'denied')
                                                    @if($booking['booking_status'] != 'canceled')
                                                        <div class="d-flex flex-column gap-2 mt-4">
                                                            <button
                                                                class="btn badge-info w-100 py-3 switch-to-cash-after-service">{{ translate('Switch to Cash after Service') }}</button>
                                                            <button
                                                                class="btn badge-danger w-100 py-3 change-booking-status">{{ translate('Cancel Booking') }}</button>
                                                        </div>
                                                    @endif
                                                @endif

                                            @else
                                                <img src="{{ asset('assets/admin-module/img/offline-payment.png') }}"
                                                     alt="Payment Icon" class="mb-3">
                            ...
                            <div class="py-3 d-flex flex-column gap-3 mb-2">
                                @if ($booking->reach_image)
                                    <div class="c1-light-bg radius-10 py-3 px-4">
                                        <div class="d-flex justify-content-start gap-2">
                                            <h4 class="mb-2">Site Selfie Image</h4>
                                        </div>

                                        <div class="py-3 px-4">
                                            <div class="d-flex flex-wrap gap-3 justify-content-lg-start">
                                                <a href="{{ $booking->on_reach_photos_full_path }}"
                                                   data-lightbox="on-reach-photos" data-title="on-reach-photos">
                                                    <img width="100" class="max-height-100"
                                                         src="{{ $booking->on_reach_photos_full_path }}"
                                                         alt="on-reach-photos">
                                                </a>
                                ...
                                @endif
                                @if ($booking->pre_work_image)
                                    <div class="c1-light-bg radius-10 py-3 px-4">
                                        <div class="d-flex justify-content-start gap-2">
                                            <h4 class="mb-2">Starting Booking Images & Videos</h4>
                                        </div>

                                        <div class="py-3 px-4">
                                            <div class="d-flex gap-3 justify-content-lg-start">
                                                @foreach ($booking->pre_work_image_full_path ?? [] as $key => $img)
                                                    <a href="{{ $img }}" data-lightbox="prework-images"
                                                       data-title="prework-images">
                                                        <img width="100" class="max-height-100"
                                                             src="{{ $img }}"
                                                             alt="prework-images">
                                                    </a>
                                        ...
                                        @if($booking->pre_videos)
                                        <div class="py-3 px-4">
                                            <div class="d-flex gap-3 justify-content-lg-start">
                                                    <a href="{{$booking->pre_videos }}" target="_blank">
                                                        <video id="videoPlayer" width="250" controls>
                                                            <source src="{{ asset('storage/' . $booking->pre_videos) }}" type="video/mp4">
                                ...
                                @if ($booking->evidence_photos)
                                    <div class="c1-light-bg radius-10 py-3 px-4">
                                        <div class="d-flex justify-content-start gap-2">
                                            <h4 class="mb-2">Completed Booking Images</h4>
                                        </div>
                                        <div class="py-3 px-4">
                                            <div class="d-flex  gap-3 justify-content-lg-start">
                                                @foreach ($booking->evidence_photos_full_path ?? [] as $key => $img)
                                                    <a href="{{ $img }}" data-lightbox="evidence-photo"
                                                       data-title="{{ translate('evidence-photo') }}">
                                                        <img width="100" class="max-height-100"
                                                             src="{{ $img }}"
                                        ...
                                        @if($booking->post_videos)
                                        <div class="py-3 px-4">
                                            <div class="d-flex gap-3 justify-content-lg-start">
                                                <a href="{{ asset('storage/' . $booking->post_videos)}}" target="_blank">
                                                    <video id="videoPlayer" width="250" controls>
                                                        <source src="{{  asset('storage/' .$booking->post_videos) }}" type="video/mp4">
                                                        Your browser does not support the video tag.
                                                    </video>
                                                </a>
                                            </div>
                                        ...
                                        <div class="py-3 px-4">
                                            <div class="media gap-2 flex-wrap">
                                                <img width="58" height="58"
                                                     ...
                                                     src="{{ $booking?->provider && $booking->provider->profile_img
                                                     ? asset('storage/provider/profile_img/'.$booking->provider->profile_img)
                                                     : asset('assets/provider-module/img/user2x.png') }}"
                                                     alt="{{ translate('provider') }}">
                                                <div class="media-body">
                                                    <a
                                                        href="{{ route('admin.provider.details', [$booking?->provider?->id, 'web_page' => 'overview']) }}">
                                                        <h5 class="c1 mb-3">
                                                            {{ Str::limit($booking->provider->full_name ?? '', 30) }}
                                                        </h5>
                                                    </a>
                                                    <ul class="list-info">
                                                        <li>
                                                            <span class="material-icons">phone_iphone</span>
                                                            <a
                                                                href="tel:{{ $booking->provider->contact_number ?? '' }}">{{ $booking->provider->contact_number ?? '' }}</a>
                                                        </li>
                                                        <li>
                                                            <span class="material-icons">map</span>
...
{{--                                            @if(in_array($booking->is_verified, ['1']))--}}
...
{{--                                                        title="Booking not allowed, Yet!"--}}
{{--                                                        style="cursor: not-allowed;">{{ translate('assign provider') }}</button>--}}
{{--                                            @endif--}}
                                        </div>
                                    @endif
                                </div>
    ...
    <div class="modal fade" id="deniedModal-{{$booking->id}}" tabindex="-1" aria-labelledby="exampleModalLabel"
         aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body pt-5 p-md-5">
                    ...
                    <div class="d-flex justify-content-center mb-4">
                        <img width="75" height="75" src="{{asset('assets/admin-module/img/icons/info-round.svg')}}"
                    ...
                    <form method="post"
                          action="{{route('admin.booking.offline-payment.verify',['booking_id' => $booking->id, 'payment_status' => 'denied'])}}">
                        @csrf
                        <div class="form-floating">
                            <textarea class="form-control h-69px" placeholder="{{translate('Type here your note')}}"
                                      name="note" id="add-your-note" maxlength="255" required></textarea>
                            ...
                            <div class="d-flex justify-content-center mt-3 gap-3">
                                ...
                                <button type="submit"
                                        class="btn btn-primary min-w-92px">{{translate('Submit')}}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
@push('script')
    ...
    <script>
        ...
        $(document).on('click', '.reassign-provider', function () {
            const id = $(this).data('provider-reassign');
            updateProvider(id);
        });
    ...
    <script>
        ...
        function updateProvider(providerId) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    $('.modal-content-data').html(response.view);
                    toastr.success('{{ translate('Assigned successfully') }}');
                    setTimeout(function () {
                        location.reload()
                    }, 600);
                },
