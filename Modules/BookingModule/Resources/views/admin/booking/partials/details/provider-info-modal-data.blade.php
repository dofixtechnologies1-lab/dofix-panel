...
<div class="modal-body p-lg-5 pt-lg-3">
    ...
    {{--    <div class="bg-soft-danger rounded py-3 px-3 title-color mb-3">--}}
    {{--        <span>--}}
    {{--            {{ translate('If you change your provider, the provider for all your upcoming bookings will also be changed.') }}--}}
    {{--        </span>--}}
    {{--    </div>--}}
    {{--    @endif--}}
    ...
    <div class="d-flex flex-column">
        ...
        {{-- Separate the matching provider from others --}}
        @foreach($providers ?? [] as $provider)
            @if($booking->provider_id == $provider->id)
                @php $matchedProvider = $provider; @endphp
            @else
                @php $otherProviders[] = $provider; @endphp
            @endif
        @endforeach

        {{-- Display the matching provider at the top --}}
        @if($matchedProvider)
            <div class="d-flex gap-2 justify-content-between align-items-center mt-4 pb-3 flex-wrap">
                <div class="media gap-2">
                     <img width="100" class="max-height-100"
                         src="{{ asset('storage/provider/profile_img/'.$matchedProvider->profile_img) }}"
                         alt="{{ translate('provider-logo') }}">
                    <div class="media-body">
                        <h5 class="mb-2">{{$matchedProvider->full_name}}</h5>
                        <div class="mb-1 fs-12"><a
                                href="tel:88013756987564">{{$matchedProvider->contact_person_phone}}</a></div>
                        <div class="provider-devider">
                            <ol class="breadcrumb fs-12 mb-0">
                                ...
                                <li class="breadcrumb-item active">{{translate('Bookings')}}
                                    - {{$matchedProvider->bookings_count}}</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div class="media gap-2">
                    <img width="100" class="max-height-100"
                     src="{{ $provider->profile_img 
                        ? asset('storage/provider/profile_img/'.$provider->profile_img) 
                        : asset('assets/provider-module/img/user2x.png') }}"
                         alt="{{ translate('provider-logo') }}">
                    <div class="media-body">
                        <h5 class="mb-2">{{$provider->full_name}}</h5>
                        <div class="mb-1 fs-12"><a href="tel:88013756987564">{{$provider->contact_person_phone}}</a>
                        </div>
                        <div class="provider-devider">
                            <ol class="breadcrumb fs-12 mb-0">
                                <li class="breadcrumb-item">
                                ...
                                <li class="breadcrumb-item active">{{translate('Bookings')}}
                                    - {{$provider->bookings_count}}</li>
                            </ol>
                        </div>
                        <h6 class="mt-2">{{translate('Address')}}
                        - {{$provider->company_address}}</h6>
                    </div>
                </div>
                <div>
                    @php
                        $isProviderBooked = DB::table('bookings')->where([
