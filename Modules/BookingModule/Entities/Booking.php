...
class Booking extends Model
{
    ...
    public function getPreWorkImageFullPathAttribute()
    {
        $preWorkImage = $this->pre_work_image ?? [];

        $defaultImagePath = asset('public/assets/admin-module/img/media/user.png');
        if (empty($preWorkImage)) {
            if (request()->is('api/*')) {
        ...
        $path = 'booking/pre_work_image/';

        return getIdentityImageFullPath(identityImages: $preWorkImage, path: $path, defaultPath: $defaultImagePath);
    }
    ...
    public function getOnReachPhotosFullPathAttribute()
    {
        //
        $image = json_decode($this->reach_image ?? []);
        $image = $image->image;
