...
trait BookingTrait
{
    ...
    public function placeBookingRequest($userId, $request, $transactionId, $newUserInfo = null, int $isGuest = 0): array
    {
        ...
        foreach ($cartData->pluck('sub_category_id')->unique() as $subCategory) {
            ...
            DB::transaction(function () use (
                ...
                // $testBookingAmt = $cartData->sum('total_cost');
                // $taxAndFeeAmt = getTaxAndFee($testBookingAmt);
                
                // $totalTestAmount = $testBookingAmt + $taxAndFeeAmt;
                // Log::info('Test Booking Amount: '.$totalTestAmount);
                ...
                $data_values = BusinessSettings::where('key_name', 'partial_payment')->first();

                $partial_amount = ($totalBookingAmount * ($data_values->live_values / 100));


                if($request['is_partial'] == 1){
                    $partialPayment = $partial_amount;
                }else{
                    $partialPayment = $totalBookingAmount;
                }
                ...
                if($partialPayment > 0){
                    if($totalBookingAmount = $request['partialPayment']){
                        $is_paid = 1;
                    }else{
                        $is_paid = 0;
                    }
                ...
                // $partialPayment = $cartData->sum('partial_amount');
                ...
                $booking->service_preference = array_key_exists("service_preference", $request->all()) && $request['service_preference'] == "onsite" ? 1 : 2;
                $booking->booking_status = 'pending';
                $booking->is_paid = $is_paid ?? 0;//$request['payment_method'] == 'cash_after_service' || $request['payment_method'] == 'offline_payment' ? 0 : 1;
                $booking->due_amt_status = $is_paid == 1 ? 1 : 0;
                ...
                if ($isPartials) {
                    $paidAmount = $partialPayment;//$customerWalletBalance;
                    ...
                    $bookingPartialPayment->paid_with = 'Razorpay';
                ...
                foreach ($cartData->all() as $datum) {
                    ...
                    $serviceId = trim((string)$datum['service_id']);

                    $service = DB::table('services')->where('id', $datum['service_id'])->first();
                    ...
                    $detail->variation_id = $datum['variation_id'];
                    $detail->save();
                ...
                $schedule->schedule = Carbon::parse($request['service_schedule']);
                $schedule->save();
                ...
                if ($booking->payment_method == 'cash_after_service') {
                    ...
                    if (isset($booking->provider_id)) {
                        ...
                        foreach ($providers as $provider) {
                            ...
                            if (!is_null($fcmToken) && $provider?->service_availability && $title && isset($bookingNotificationStatus) && $bookingNotificationStatus['push_notification_booking'] && sendDeviceNotificationPermission($booking?->provider_id)) device_notification($fcmToken, $title, null, null, $booking->id, 'booking');
                        }
                    }
                }
            });
    ...
    public function calculateCommissionDetails($booking): array
    {
        ...
        $bookDetails = BookingDetail::where('booking_id', $bookingId)->get();
        ...
        $adminCommission = $totalConvenienceFee ?? 0;
