...
class BookingController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'limit' => 'required|numeric|min:1|max:200000',
        ...
        $bookings = $this->booking
            ->with(['repeat', 'customer', 'service_address', 'subCategory:id,name', 'booking_offline_payments' => function ($query) {
                    $query->first() ?? [];
            }])
            ...
            ->when($request['booking_status'] == 'all', function ($query) use ($providerId, $request, $maxBookingAmount) {
                $query->where(function ($query) use ($providerId) {
                    ...
                    ->orWhere(function ($query) use ($providerId, $request, $maxBookingAmount) {
    ...
    public function statusBookings(Request $request): JsonResponse
    {
        ...
        $bookings = $this->booking
            ->with(['customer', 'repeat', 'subCategory', 'service_address'])
            ->where(['provider_id' => $request->user()->provider->id])
            ->search(base64_decode($request['string']), ['readable_id'])
    ...
    public function todayBookings(Request $request): JsonResponse
    {
        ...
        //bookings list
        $bookings = $this->booking->whereDate('service_schedule', Carbon::today())
            ->with(['repeat', 'customer', 'service_address', 'subCategory:id,name', 'booking_offline_payments' => function ($query) {
                    $query->first() ?? [];
    ...
    public function homeStats(Request $request): JsonResponse
    {
        ...
        $total_bookings = $this->booking
            ->whereDate('service_schedule', Carbon::today())
            ->where('provider_id', $request->user()->provider->id)->count();

        $bookings_count['total_bookings'] = $total_bookings;
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        $booking = $this->booking->with([
            'detail.service', 'schedule_histories.user', 'status_histories.user', 'service_address', 'customer',
    ...
    public function statusUpdate(Request $request)
    {
        ...
        if (isset($booking)) {
            ...
            if ($request['booking_status'] == 'completed') {
                ...
                if ($request->has('evidence_photos')) {
                    foreach ($request->evidence_photos as $image) {
                        ...
                        $imageName = file_uploader('booking/evidence/', $extension, $image);
            ...
            if ($request['booking_status'] == 'ongoing') {
                if ($request->has('reach_image')) {
                    $image = $request->reach_image;
                    $extension = $image->getClientOriginalExtension(); // or $image->extension();
                    $imageName = file_uploader('booking/reach_image/', $extension, $image);
                    $reach_image = ['image' => $imageName, 'storage' => getDisk()];
                }
            }
            ...
            if ($request['booking_status'] == 'ongoing') {
                ...
                if ($request->has('pre_work_image')) {
                    foreach ($request->pre_work_image as $image) {
                        $extension = $image->getClientOriginalExtension(); // or $image->extension();
                        $imageName = file_uploader('booking/pre_work_image/', $extension, $image);
                        $pre_work_image[] = ['image' => $imageName, 'storage' => getDisk()];
                    }
                }
            ...
            $preWorkVideoPath = null;
            if ($request->booking_status == 'ongoing') {
                if ($request->hasFile('pre_work_video')) {
//                        $videoFile = $request->file('pre_work_video');   // <-- Correct
//                        $originalExtension = $videoFile->getClientOriginalExtension();
//                        $fileName = Str::uuid() . '.' . $originalExtension;
//                        $disk = 'public';
//                        $path = $videoFile->storeAs('booking/videos', $fileName, $disk);
//                        $preWorkVideoPath = $path;
                    $videoFile = $request->file('pre_work_video');
                    $originalExtension = $videoFile->getClientOriginalExtension();
                    $fileName = Str::uuid() . '.' . $originalExtension;
                    $disk = 'public';
                    $path = $videoFile->storeAs('booking/videos', $fileName, $disk);
            ...
            $postWorkVideoPath = null;
            if ($request->booking_status == 'completed') {

                if ($request->hasFile('post_work_video')) {

                    $videoFile = $request->file('post_work_video');
            ...
            if (!empty($pre_work_image)) {
                $booking->pre_work_image = $pre_work_image;
            }
            if (!empty($preWorkVideoPath)) {
                $booking->pre_videos = $preWorkVideoPath;
            }

            if (!empty($postWorkVideoPath)) {
                $booking->post_videos = $postWorkVideoPath;
            }


            if (!empty($reach_image))
                $booking->reach_image = $reach_image;
