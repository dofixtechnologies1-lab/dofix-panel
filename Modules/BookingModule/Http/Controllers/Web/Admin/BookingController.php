...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, ServiceEnquiry $serviceEnquiry, BookingRepeatDetails $bookingRepeatDetail, BookingRepeatHistory $bookingRepeatHistory, BookingRepeat $bookingRepeat, BookingDetail $bookingDetails, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, SubscribedService $subscribedService, Category $category, Zone $zone, Serviceman $serviceman, Provider $provider, UserAddress $userAddress, BookingAdditionalInformation $bookingAdditionalInformation)
    {
    ...
    public function serviceEnquiryList(Request $request)
    {
        $search = trim($request->input('search', ''));
        $status = $request->input('status', 'all');
        $queryParam = ['search' => $search, 'status' => $status];

        $enquiries = $this->serviceEnquiry
            ...
            ->when($status !== 'all', function ($query) use ($status) {
                return $query->where('status', $status == 'done' ? 1 : 0);
            })
            ->latest()
            ->paginate(pagination_limit())
            ->appends($queryParam);

        return view('bookingmodule::admin.booking.service_enq_list', compact('enquiries', 'search', 'status'));
    }


    public function enquiryView($id)
    {
        $enquiry = ServiceEnquiry::findOrFail($id);
        $service = Service::find($enquiry->service_id);

        $providersObj = Provider::where('zone_id', $enquiry->zoneID)->get();

        return view('bookingmodule::admin.booking.enquiry_view', compact('enquiry', 'service', 'providersObj'));
    }
    ...
    public function enquiryStatus(Request $request, $id)
    {
        ...
        $enquiry->provider_id = $validated['provider_id'];
        $enquiry->save();
    ...
    public function details($id, Request $request)
    {
        ...
        if ($request->web_page == 'details') {
            ...
            $booking = $this->booking->with(['detail.service' => function ($query) {
                $query->withTrashed();
            }, 'detail.service.category', 'detail.service.subCategory',
                'detail.variation', 'customer', 'provider', 'service_address',
                'serviceman', 'service_address', 'status_histories.user'])
                ->find($id);
            ...
            $restrictedWalletBalance = BusinessSettings::where('key_name', 'minimum_wallet_amount_for_listing')->first()->live_values;
             $providers = $this->provider::with(['owner' => function ($q) {
                $q->with(['account']);
            }])->where('service_id', $booking->category_id)
                ->whereHas('owner', function ($q) use ($restrictedWalletBalance) {
                    $q->whereHas('account', function ($q) use ($restrictedWalletBalance) {
                        $q->where('account_receivable', '>=', $restrictedWalletBalance);
                    });
                })
                ->when($request->has('search'), function ($query) use ($request) {
                    $keys = explode(' ', $request['search']);
                ...
                // Attenstion :  Need to enable latter
     ...
     * @throws AuthorizationException
     */
    public function statusUpdate($bookingId, Request $request)
    {
     ...
     * @return Renderable
     */
    public function customerInvoice($id): Renderable
    {
        // App::setLocale($lang);
        $booking = $this->booking->with(['detail.service' => function ($query) {
            $query->withTrashed();
