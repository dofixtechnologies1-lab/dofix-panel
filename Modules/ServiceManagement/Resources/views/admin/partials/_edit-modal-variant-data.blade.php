<form method="POST" action="{{ route('admin.service.update-modal-variant') }}" class="row g-3" enctype="multipart/form-data">
    @csrf
    ...
    <input type="file" class="form-control" name="var_image" accept="image/*">

    @if(!empty($variants->cover_image))
        <div class="mt-2">
            <img src="{{ asset('storage/service/variant/' . $variants->cover_image) }}" alt="Variant Image" 
                 style="max-height: 80px; border-radius:6px; object-fit:cover; display:block;">

            {{-- Add margin-top to separate button from image --}}
            <div class="mt-2 text-end">
                <input type="submit" class="btn btn-primary" value="Update">
            </div>
        </div>
    @else
        {{-- If no image, still show button below input --}}
        <div class="mt-2 text-end">
            <input type="submit" class="btn btn-primary" value="Update">
        </div>
    @endif
