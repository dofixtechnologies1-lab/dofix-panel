...
@if(session()->has('variations'))
    @foreach(session('variations') as $key=>$item) 
        <tr>
            ...
            @endforeach
            <td>
                {{-- Show cover image from session --}}
                @if(!empty($item['var_image']) && file_exists(public_path('storage/service/variant/' . $item['var_image'])))
                    <img src="{{ asset('storage/service/variant/' . $item['var_image']) }}" 
                         alt="Cover Image" 
                         style="max-height: 80px; border-radius: 6px; display:block; margin-bottom:5px;">
                 @else
