...
class ServiceController extends Controller
{
    ...
    public function create(Request $request): View|Factory|Application
    {
        ...
        return view('servicemanagement::admin.create', compact('categories', 'zones', 'taxes'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $service->service_charges = $request->input('service_fee', 0);
        $service->save();
        ...
        if ($variations) {
            ...
            foreach ($variations as $item) {
                ...
                foreach ($zones as $zone) {
                    ...
                    if (!empty($data[$item['variant_key'] . '_' . $zone->id . '_price']) && $data[$item['variant_key'] . '_' . $zone->id . '_price'] != 0) {
                        $variationFormat[] = [
                            ...
                            'cover_image' => $item['var_image'] ?? null,
                        ];
                    }
                }
            }
    ...
    public function edit(string $id)
    {
        ...
        if (isset($service)) {
            $zoneIds = $service->category->zones->pluck('id');
            $editingVariants = $service->variations->whereIn('zone_id',$zoneIds)->values();
            $service->variations = $editingVariants;
...
//            return $service;
            ...
            return view('servicemanagement::admin.edit', compact('categories', 'zones', 'service', 'tagNames', 'taxes'));
        }
...
//        return $request;
        ...
        foreach ($data['variants'] as $k => $item) {
            ...
            if (!$imagePath) {
                $imagePath = $request->input('old_var_image.' . $item) ?? null;
            }
...
// }
    ...
    //     }
        ...
        $variation = [
            ...
            'convenience_gst' => $request['convenience_gst'],
            'aggregator_fee' => $request['aggregator_fee'],
            'aggregator_gst' => $request['aggregator_gst'],
    ...
    public function ajaxRemoveVariant($variant_key)
    {
        ...
        $existingData = session()->has('variations') ? session('variations') : [];
         ...
         $filtered = collect($existingData)->filter(function ($values) use ($variant_key) {
            if ($values['variant_key'] == $variant_key) {

                // delete image
                if (!empty($values['var_image']) && Storage::disk('public')->exists($values['var_image'])) {
                    Storage::disk('public')->delete($values['var_image']);
                }
    ...
    public function ajaxDeleteDbVariant($variant_key, $service_id)
    {
        ...
        $variant = Variation::where('variant_key', $variant_key)->where('service_id', $service_id)->first();

        if ($variant && !empty($variant->cover_image) && Storage::disk('public')->exists($variant->cover_image)) {
            Storage::disk('public')->delete($variant->cover_image);
        }
    
        // Delete the record directly using query (bypasses any model issues)
        Variation::where('variant_key', $variant_key)->where('service_id', $service_id)->delete();
    ...
    public function ajaxDbVariant($variant_key, $service_id)
    {
        $zones = session()->has('category_wise_zones') ? session('category_wise_zones') : $this->zone->ofStatus(1)->latest()->get();
        $variants = $this->variation->where(['id' => $service_id])->first();

        $taxes = DB::table('taxes')->get();

        return response()->json(['flag' => 1, 'template' => view('servicemanagement::admin.partials._edit-modal-variant-data', compact('zones', 'variants', 'taxes'))->render()]);
    }
    ...
    public function updateVariant(Request $request)
    {
        ...
        $variations = Variation::where('service_id', $request->service_id)->where('variant_key', $request->variant_key)->get();

        foreach ($variations as $variation) {
            
            $newName = $variation->cover_image; // default old image

            if ($request->hasFile('var_image')) {
                // delete old file if exists
                if ($newName && \Storage::disk('public')->exists($newName)) {
                ...
                $file = $request->file('var_image');
                $filename = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
                $newName = $filename . '_' . date('Ymd_His') . '.' . $file->getClientOriginalExtension();
                $coverImagePath = $file->storeAs('service/variant', $newName, 'public');
            }

            Variation::where('id', $variation->id)->update([
                'variant' => $request->variant,
                'variant_key' => $request->variant,
                'mrp_price' => $request->mrp_price,
        ...
        return redirect()->back()->with(Toastr::success(translate(DEFAULT_UPDATE_200['message'])));
    }
