...
class ServiceController extends Controller
{
    ...
    private function variationMapper($services, $request = null)
    {
        $services->map(function ($service) use ($request) {
    ...
    private function variationsAppFormat($service, $request)
    {
        $formatting = [];
        //        $filtered = $service['variations']->where('zone_id', Config::get('zone_id'));
        //        $filtered = $service['variations']->where('zone_id', $request->header('zoneID'));
        return $filtered = collect($service['variations'])
            ->where('zone_id', $request->header('zoneID'))
            ->values()
            ->toArray();

        // commented because not in use in customer app
        //            $formatting['zone_id'] = Config::get('zone_id');
        //            $formatting['default_price'] = !empty($filtered)
        //                ? $filtered[0]['price']
        //                : 0;
