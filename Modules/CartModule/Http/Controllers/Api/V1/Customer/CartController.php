...
class CartController extends Controller
{
    ...
    public function __construct(Cart $cart, Service $service, Variation $variation, User $user, Provider $provider, Guest $guest, Request $request, Booking $booking, Coupon $coupon, BusinessSettings $business_setting)
    {
        $this->cart = $cart;
    ...
    public function addToCart(Request $request)
    {
        ...
        if (isset($variation)) {
            ...
            $subtotal += $service->min_bidding_price;
            ...
            // $tax = round((($subtotal - $applicableDiscount) * $service['tax']) / 100, 2);
            $tax = $service['tax'];
            ...
            $total_cost = round($subtotal - $basicDiscount, 2);
            ...
            $cart->service_cost = $subtotal;//$variation->price;
            ...
            $cart->total_cost = round($subtotal - $basicDiscount, 2); //round($subtotal - $basicDiscount - $campaignDiscount + $tax, 2);
            ...
            return response()->json(response_formatter(DEFAULT_STORE_200, $cart), 200);
        }
...
//        $totalCost = round($totalCost, 2);
        $referralAmount = 0.00;
        $tax_amount = 79;
        // $tax_test_amt = getTaxAndFee($totalCost);
        // Log::info('Test Booking Amount: '.$tax_test_amt);
        

        return response()->json(response_formatter(DEFAULT_200, ['total_cost' => $totalCost, 'referral_amount' => $referralAmount, 'wallet_balance' => with_decimal_point($walletBalance), 'tax_amount' => $tax_amount, 'cart' => $cart]), 200);
    }
