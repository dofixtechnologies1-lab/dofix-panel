...
if (!function_exists('completeBookingTransactionForCashAfterService')) {
    function completeBookingTransactionForCashAfterService($booking): void
    {
        ...
        if ($bookingType) {
            ...
            $bookDetails = BookingDetail::where('booking_id', $booking->id)->get();
            $totalConvenienceFee = 0;
            foreach ($bookDetails as $detail) {
                $variation = Variation::find($detail->variation_id);
                $totalConvenienceFee += $variation?->convenience_fee ?? 0;
            }
            ...
            $admin_commission = $totalConvenienceFee;
        }
...
//            $account->received_balance += $booking_amount_without_commission - (($booking_amount_without_commission * $share) / 100);
            $account->received_balance += round($booking->total_booking_amount - (($booking->total_booking_amount * $share) / 100), 2);
            $account->account_receivable -= round((($booking->total_booking_amount * $share) / 100), 2);
//            $account->account_payable += round((($booking->total_booking_amount * $share) / 100), 2);
            $account->save();
            Log::info('amount_check ' . $booking->total_booking_amount . ' ' . $share);

            Transaction::create([
                'ref_trx_id' => null,
                'booking_id' => $booking['id'],
                'trx_type' => TRX_TYPE['payable_amount'],
                'debit' => (($booking_amount_without_commission * $share) / 100),
                'credit' => 0,
                'balance' => $account->received_balance,
...
//                $account->account_payable += $booking['extra_fee'];
            ...
            //expense (provider)
            if ($promotional_cost_by_provider) {
                $account = Account::where('user_id', $provider_user_id)->first();
                $account->total_expense += $promotional_cost_by_provider;
                $account->save();
            }
...
if (!function_exists('completeBookingRepeatTransactionForCashAfterService')) {
    function completeBookingRepeatTransactionForCashAfterService($booking): void
    {
        ...
        if ($bookingType) {
            ...
            foreach ($bookDetails as $detail) {
                $variation = Variation::find($detail->variation_id);
                $totalConvenienceFee += $variation->convenience_fee;
            }
        }
        //admin promotional cost will be deducted from admin commission
    ...
    function completeBookingTransactionForPartialDigital($booking): void
    {
        ...
        DB::transaction(function () use ($booking, $admin_user_id, $provider_user_id, $admin_commission, $booking_amount_without_commission, $promotional_cost_by_admin, $promotional_cost_by_provider) {
            ...
            $primary_transaction = Transaction::create([
                ...
                'credit' => 0,
                'balance' => !empty($account->balance_pending) ? $account->balance_pending : 0,
                'from_user_id' => $admin_user_id,
                'to_user_id' => $provider_user_id,
                'from_user_account' => ACCOUNT_STATES[0]['value'],
                'to_user_account' => null
            ...
            //Provider transactions (+receivable)
            $account = Account::where('user_id', $provider_user_id)->first();
            $adminDefaultCommission = DB::table('business_settings')->where('key_name', 'default_commission')->first()->live_values;
            $account->account_receivable += round($booking['total_booking_amount'] - (($booking['total_booking_amount'] * $adminDefaultCommission) / 100), 2);
            $account->received_balance += round($booking['total_booking_amount'] - (($booking['total_booking_amount'] * $adminDefaultCommission) / 100), 2);
            $account->account_payable += round((($booking['total_booking_amount'] * $adminDefaultCommission) / 100), 2);
            $account->save();
            ...
            Transaction::create([
                ...
                'trx_type' => TRX_TYPE['receivable_amount'],
                'debit' => 0,
                'credit' => round($booking['total_booking_amount'] - (($booking['total_booking_amount'] * $adminDefaultCommission) / 100), 2),
                'balance' => $account->account_receivable,
                'from_user_id' => $admin_user_id,
                'to_user_id' => $provider_user_id,
                'from_user_account' => null,
...
//                    $account->account_payable += $booking_amount_without_commission;
            ...
            if (!empty($account->account_payable)) {
                Transaction::create([
                    'ref_trx_id' => $primary_transaction['id'],
                    'booking_id' => $booking['id'],
                    'trx_type' => TRX_TYPE['payable_amount'],
...
if (!function_exists('completeBookingTransactionForDigitalPaymentAndExtraService')) {
    function completeBookingTransactionForDigitalPaymentAndExtraService($booking): void
    {
        ...
        DB::transaction(function () use ($booking, $admin_user_id, $provider_user_id, $admin_commission, $booking_amount_without_commission, $promotional_cost_by_admin, $promotional_cost_by_provider) {
            ...
            $account->received_balance += $provider_earning_for_digital;
            $account->save();
...
if (!function_exists('shiftWalletRechargeTransaction')) {
    function shiftWalletRechargeTransaction($amount, $provider_id)
    {
        $totalAmount = $amount;
        ...
        $account = Account::where('user_id', $providerUserId)->first();
        if (!$account) {
        ...
        $account->account_receivable += $totalAmount;
        $account->save();

        $transaction = Transaction::create([
            'ref_trx_id' => null,
            'trx_type' => 'wallet_recharge',  // fix typo if any
            'debit' => 0,
            'credit' => $totalAmount,
            'balance' => $account->account_receivable, // check if this is the right balance
            'from_user_id' => $providerUserId,
            'to_user_id' => $providerUserId,
            'from_user_account' => null,
            'to_user_account' => ACCOUNT_STATES[1]['value'],
