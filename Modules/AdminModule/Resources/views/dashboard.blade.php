                ...
                @if(access_checker('dashboard'))
                    <div class="row mb-4 g-4">
                        <div class="col-lg-3 col-sm-6">
                            <a href="{{url('admin/transaction/list?trx_type=all')}}">
                                <div class="business-summary business-summary-customers">
                                    <h2>{{with_currency_symbol(data_get($data[0], 'top_cards.total_commission_earning', 0) +
                                        data_get($data[0], 'top_cards.total_subscription_earning', 0) +
                                        data_get($data[0], 'top_cards.total_system_earning', 0))}}</h2>
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <a href="{{url('admin/service/list?status=all')}}">
                                <div class="business-summary business-summary-providers">
                                    <h2>{{data_get($data[0], 'top_cards.total_Services', 0)}}</h2>
                        ...
                        <div class="col-lg-9">
                            <div class="card earning-statistics">
                                <div class="card-body ps-0">
                                    <div
                                        class="ps-20 d-flex flex-wrap align-items-center justify-content-between gap-3">
                                        ...
                                        @foreach($data[3]['bookings'] as $booking)
                                            <li class="d-flex flex-wrap gap-2 align-items-center justify-content-between cursor-pointer recent-booking-redirect"
                                                ...
                                                <div class="media align-items-center gap-3">
                                                    ...
                                                    <div class="media-body ">
                                                        <h5 class="d-flex align-items-center">{{translate('Booking')}}
                                                            # {{$booking->readable_id}}
                                                            @if($booking->is_repeated)
                                                                <img
                                                                    src="{{ asset('assets/admin-module/img/icons/repeat.svg') }}"
                                                                    class="rounded-circle repeat-icon m-1"
...
@push('script')
    ...
    <script>
        ...
        var options = {
            series: [
                {
                    name: "{{translate('booking_amount')}}",
                    data: @json($chart_data['total_earning'])
                },
                {
                    name: "{{translate('month_wise_bookings')}}",
        ...
        function update_chart(year) {
            ...
            $.getJSON(url, function (response) {
                chart.updateSeries([{
                    name: "{{translate('month_wise_bookings_amount')}}",
