...
class CategoryController extends Controller
{
    ...
    public function childes(Request $request): JsonResponse
    {
        ...
        if($request['id'] == '40dfb6c2-df7b-4708-acc0-e096a1e0ff8e'){
            ...
            ->where('parent_id', $request['id'])->orderByRaw("FIELD(name, 'AC Servicing', 'AC Repairs', 'AC Installation/Uninstallation', 'Addon Services')")
            ->paginate($request['limit'], ['*'], 'offset', $request['offset'])->withPath('');
    ...
    public function topRatedFeatured(Request $request): JsonResponse
    {
        // $validator = Validator::make($request->all(), [
        //     'limit' => 'required|numeric|min:1|max:200',
        //     'offset' => 'required|numeric|min:1|max:100000'
        ...
        $categories = $this->category->with(['zones', 'services_by_category.variations', 'services_by_category' => function ($query) {
            ...
            // ->paginate($request['limit'], ['*'], 'offset', $request['offset'])->withPath('');
            ->first();
        // dd($categories->services_by_category);
        // foreach ($categories as $category) {
        $categories->data = self::variationMapper($categories->services_by_category);
        // }

        return response()->json(response_formatter(DEFAULT_200, $categories), 200);
    }

    public function quickRepairFeatured(Request $request)
    {
...
if($categories?->services_by_category){
        $categories->data = self::variationMapper($categories->services_by_category);
}
