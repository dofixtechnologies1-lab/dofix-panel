...
class CategoryController extends Controller
{
    ...
    public function update(Request $request, string $id): JsonResponse|RedirectResponse
    {
        ...
        // 🔹 MINIMUM BALANCE (NEW)
        $category->minimum_balance = $request->filled('minimum_balance')
            ? (int) $request->minimum_balance
            : null;
        
        // 🔹 IMAGE
        if ($request->has('image')) {
            $category->image = file_uploader(
                'category/',
                'png',
                $request->file('image'),
                $category->image
    ...
    public function top_ratedUpdate(Request $request, $id): JsonResponse
    {
        $this->authorize('category_manage_status');
        $category = $this->category->where('id', $id)->first();
        $this->category->where('id','!=',$id)->update(['top_rated' =>0]);
        $this->category->where('id', $id)->update(['top_rated' => !$category->top_rated]);

        return response()->json(response_formatter(DEFAULT_UPDATE_200), 200);
    }


    public function quick_repairUpdate(Request $request, $id): JsonResponse
    {
        $this->authorize('category_manage_status');
        $category = $this->category->where('id', $id)->first();

        $this->category->where('id','!=',$id)->update(['quick_repair' =>0]);
        $this->category->where('id', $id)->update(['quick_repair' => !$category->quick_repair]);
    ...
    // public function categoryHomeCreate(Request $request)
    // {   
    //     $zones = $this->zone->where('is_active', 1)->withoutGlobalScope('translate')->get();
    //     $home_category_slider=BusinessSettings::where('key_name','home_category_slider')->first();
    //     $category = $this->category->latest()->get();

    //     return view('categorymanagement::admin.home_category_slider.create',compact('home_category_slider','zones','category'));
    // }   
